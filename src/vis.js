...
// === Touch support ===
// Snapshot-based: capture state at gesture start, derive current from delta.
// Avoids compounding floating-point drift on every touchmove.
let touchMode = ''; // 'pan' | 'pinch' | ''
let pinchStartDist = 0, pinchStartScale = 0;
let pinchStartMid = null, pinchStartPanX = 0, pinchStartPanY = 0;
...
interLayer.addEventListener('touchmove', e => {
  ...
  if (e.touches.length === 1 && touchMode === 'pan') {
    ...
    // Compute new scale from snapshot ratio (no drift)
    const newScale = Math.max(0.05, Math.min(5, pinchStartScale * (dist / pinchStartDist)));

    // World point under midpoint at gesture start
    const wx = (pinchStartMid.x - cx) / pinchStartScale - pinchStartPanX;
    const wy = (pinchStartMid.y - cy) / pinchStartScale - pinchStartPanY;

    // Keep that world point under current midpoint
    scale = newScale;
...
interLayer.addEventListener('touchend', e => {
  ...
  if (e.touches.length === 0) {
    // Tap / double-tap detection (only if finger didn't move)
    if (!touchMoved) {
      ...
      if (now - lastTapTime < 400 && Math.hypot(dx, dy) < 30) {
        ...
        if (idx >= 0) window.open(nodes[idx].finalUrl || nodes[idx].url, '_blank');
        lastTapTime = 0;
      } else {
        lastTapTime = now;
        lastTapX = ct.clientX;
    ...
    // Went from pinch to 1 finger — switch to pan from current state
    touchMode = 'pan';
    const t = e.touches[0];
    dragStartX = t.clientX; dragStartY = t.clientY;
    panStartX = panX; panStartY = panY;
    ...
    // Finger added back — restart pinch from current state
    startPinch(e);
  }
}, { passive: false });
